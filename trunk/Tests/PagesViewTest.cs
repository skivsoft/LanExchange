...
namespace Tests
{
    ...
    public class PagesViewTest
    {
        ...
        [TestMethod]
        public void PanelView_FilterText()
        {
            using (var View = new PagesView())
            {
                var Presenter = View.GetPresenter();
                var Model = Presenter.GetModel();
                var Items = new PanelItemList("test1");
                Model.AddTab(Items);
                View.SelectedIndex = 0;
                PanelView PV = View.GetActivePanelView();
                Assert.IsNotNull(PV);
                PV.FilterText = "test";
                //Assert.AreEqual(true, PV.FilterVisible);
                Assert.AreEqual("test", PV.FilterText);
                Assert.AreEqual("test", Items.FilterText);
            }
        }

        [TestMethod]
        public void PanelItemList_FilterText()
        {
            using (var View = new PagesView())
            {
                var Presenter = View.GetPresenter();
                ...
                PanelView PV = View.GetActivePanelView();
                Assert.IsNotNull(PV);
                Items.FilterText = "test";
                //Assert.AreEqual(true, View.FilterVisible);
                Assert.AreEqual("test", Items.FilterText);
                Assert.AreEqual("test", PV.FilterText);
            }
        }
